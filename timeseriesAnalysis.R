...
# Cumulative number of SARS-COV-2 cases
df = read.csv("covid19za_provincial_cumulative_timeline_confirmed.csv",header = T,sep = ',')

tiff(file='SA_sars-cov-2-infections-cov1trend.tif', width=648, height=494)

DF = data.frame(Time=c(time(df$date)),FS=c(df$FS),EC=c(df$EC), WC=c(df$WC),GP=c(df$GP),KZN=c(df$KZN),LP=c(df$LP),MP=c(df$MP),NW=c(df$NW),NC=(df$NC))
ggplot(data = DF, aes(x=Time, y=value, color=Province )  )+
  ylab('Number of infections')+
  geom_line(aes(y=FS , col='Free State'), size=2, alpha=.5)   +
  geom_line(aes(y=EC, col='Eastern Cape'),  size=2, alpha=.6)   +
  geom_line(aes(y=WC, col='Western Cape'),  size=2, alpha=.7)   +
  geom_line(aes(y=GP, col='Gauteng '),  size=2, alpha=.8)   +
  geom_line(aes(y=KZN, col='Kwazulu Natal'),  size=2, alpha=.9)   +
  geom_line(aes(y=LP, col='Limpopo'),  size=2, alpha=.5)   +
  geom_line(aes(y=MP, col='Mpumalanga'),  size=2, alpha=.5)   +
  geom_line(aes(y=NW, col='North West'),  size=2, alpha=.5)   +
  geom_line(aes(y=NC, col='Northern Cape'),  size=2, alpha=.5)   +
...
#cumulative number of deaths in South Africa
deaths = read.csv("covid19za_provincial_cumulative_timeline_deaths.csv",header = T,sep = ',')

tiff(file='comulativeDeaths.tif', width=648, height=494)

DF2 = data.frame(Time=c(time(deaths$date)),FS=c(deaths$FS),EC=c(deaths$EC), WC=c(deaths$WC),GP=c(deaths$GP),KZN=c(deaths$KZN),LP=c(deaths$LP),MP=c(deaths$MP),NW=c(deaths$NW),NC=c(deaths$NC))
ggplot(data = DF2, aes(x=Time, y=value, color=Province )  )+
...
#Variants time series analysis
#cumulative number of deaths in South Africa
variants = read.csv("q1Variants2020.csv",header = T,sep = ',')

tiff(file='variantFreq.tif', width=648, height=494)

DFv = data.frame(Time=c(time(variants$date)),Variant_type =c(variants$Variant_type),Infected_individuals=c(variants$Infected_individuals), Year=c(variants$Year))
ggplot(data = DFv, aes(x=Time, y=Infected_individuals, color=Variant_type )  )+
  ylab('Variant frequencies')+
  geom_line(aes(y=Infected_individuals , col='Infected individuals'), size=2, alpha=.5) +
  theme_cowplot()
dev.off()
#==========================================================================

confirmedraw <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv",header = T,sep = ',')
str(confirmedraw) # Check latest date at the end of data
deathsraw <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv",header = T,sep = ',')
recoveredraw <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv",header = T,sep = ',')
...
library(ggplot2)
# World confirmed
ggplot(world, aes(x=date, y=confirmed)) + geom_bar(stat="identity", width=0.5) +
  theme_cowplot() +
...
# World confirmed
ggplot(world, aes(x=days, y=confirmed)) + geom_point() +
  theme_cowplot() +
...
countryselection <- country %>% filter(Country.Region==c("Kenya", "Rwanda", "Uganda", "Tanzania","Ethiopia","South Africa"))
ggplot(countryselection, aes(x=days, y=confirmed, colour=Country.Region)) + geom_line(size=2) +
  theme_classic() +
...
countryselection %>% gather("Type", "Cases", -c(date, days, Country.Region)) %>%
  ggplot(aes(x=days, y=Cases, colour=Country.Region)) + geom_line(size=2) +
  theme_classic() +
